...
class ScrollablePositionedList extends StatefulWidget {
  ...
  /// Index of an item that is considered the center of the scrollable.
  ...
  /// Make sure you're not using [separatorBuilder] when
  /// using [findChildIndexCallback].
...
class ItemScrollController {
  ...
  /// See [_ScrollablePositionedListState.resetInitialCenterIndex].
  /// Only applicable for bidirectional lists.
  void resetInitialScrollIndex([int value]) {
    _scrollableListState._resetInitialCenterIndex(value);
  }
...
class _ScrollablePositionedListState extends State<ScrollablePositionedList>
  ...
  var _isTransitioningNotifier = ValueNotifier<bool>(false);
  bool get _isTransitioning => _isTransitioningNotifier.value;

  bool get _hasCenterIndex =>
      _initialCenterIndex != null && widget.centerIndex != null;

  int _initialCenterIndex;
  int get _centerDelta =>
      _hasCenterIndex ? widget.centerIndex - _initialCenterIndex : 0;

  /// Resets the center index to [widget.centerIndex] or [value], if provided.
  /// Used when the center of the view changes (for example, scrolling).
  void _resetInitialCenterIndex([int value]) {
    _initialCenterIndex = value ?? widget.centerIndex;
  }

  void _isTransitioningListener() {
    widget.itemPositionsNotifier?.transitioningNotifier?.value =
        _isTransitioning;
  }
  ...
  void initState() {
    ...
    _isTransitioningNotifier.addListener(_isTransitioningListener);
  }
  ...
  void dispose() {
    ...
    secondary.itemPositionsNotifier.itemPositions
        .removeListener(_updatePositions);
    _isTransitioningNotifier.removeListener(_isTransitioningListener);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    final cacheExtent = MediaQuery.of(context).size.height * _screenScrollCount;
            ...
            child: FadeTransition(
              ...
              child: NotificationListener<ScrollNotification>(
                ...
                child: PositionedList(
                  ...
                  positionedIndex: primary.target + _centerDelta,
              ...
              child: FadeTransition(
                ...
                child: NotificationListener<ScrollNotification>(
                  ...
                  child: PositionedList(
                    ...
                    positionedIndex: secondary.target + _centerDelta,
  ...
  void _updatePositions() {
    ...
    widget.itemPositionsNotifier?.viewportDimension?.value =
        primary.scrollController.position.viewportDimension;
  }
}
